...
import { CompanyExpenseService } from '../../core/services/company-expense.service';
...
@Component({
  selector: 'app-company-expenses-list',
  templateUrl: './company-expenses-list.component.html',
  styleUrls: ['./company-expenses-list.component.scss']
})
export class CompanyExpensesListComponent implements OnInit, AfterViewInit {
  displayedColumns: string[] = ['date', 'label', 'montant', 'actions'];
  dataSource = new MatTableDataSource<CompanyExpense>([]);
  globalFilter: string = '';
  columnFilters: Record<string, string> = {};
  columnPlaceholders: Record<string, string> = {
    date: 'companyExpenses.columns.date',
    label: 'companyExpenses.columns.label',
    montant: 'companyExpenses.columns.montant'
  };
  @ViewChild(MatPaginator) paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;

  constructor(
    private service: CompanyExpenseService,
  ...
  openCreate(){ this.dialog.open(CompanyExpensesDialogComponent, { width: '420px', data: { mode: 'create' } }); }

  openEdit(item: CompanyExpense){ this.dialog.open(CompanyExpensesDialogComponent, { width: '420px', data: { mode: 'edit', item } }); }

  delete(item: CompanyExpense){
