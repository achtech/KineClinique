...
export class PrescriptionService {
  ...
  constructor() {
    const demo: Prescription[] = Array.from({length: 30}, (_, i) => ({
      id: `presc-${i+1}`,
      patientId: `pat-${(i%10)+1}`,
      prescriber: `Dr. ${['Martin','Durand','Lefevre','Petit','Moreau'][i%5]}`,
      sessionsPrescribed: 10 + (i%5),
      sessionsUsed: i%10,
      createdAt: new Date(Date.now() - i*86400000).toISOString(),
      documentUrl: i%4===0 ? `https://demo.com/doc${i+1}.pdf` : undefined
    }));
    this.prescriptions$.next(demo);
  }
